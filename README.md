...
Install the dependencies using a node package manager of your choice, e.g. `npm install`.

## Development

If you want to adjust the existing code you can start up a development server using `npm run dev`. The stream will be captured this way as well.
...
Before deploying you need to set some environment variables within a `.env` file and trusted CORS origins. Once you are done you can build and deploy this project by using `npm run build`. This will create a production-ready [Node](https://nodejs.org/en/about) environment.

Afterwards you can run the application by using `node --env-file=.env build/index.js` (Requires Node 20.6.0 or higher!)
...
Sveltekit uses a built-in protection against Cross-Site Request Forgery (CSRF) attacks. We need to set trusted origins (or disable it) so submissions like the login form work correctly. To set them please to the following:
...
1. Install globally using `npm install pm2 -g`
2. Start the application using `pm2 start build/index.js --name "private-stream" --node-args="--env-file=.env"`
3. Save the process using `pm2 save`
4. Add it to autostart using `pm2 startup`
